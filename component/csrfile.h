/*
 * @Author: 苗金标
 * @Date: 2023-03-24 19:23:18
 * @LastEditors: 苗金标
 * @LastEditTime: 2023-03-26 15:26:40
 * @Description: 
 */
#pragma once
#include "common.h"
...
namespace component{
    class csrfile : if_print_t{
        private:
            ...
            //建立的是指令寄存器中的地址和csr文件夹中定义的csr之间的关系
        ...
        public:
                ...
                while (!sync_request_q.empty())
                {
                    ...
                    switch(t_req.req){
                        ...
                        case sync_request_type_t::write_sys:
                            ...
                            break;
                        default:
                            std::cout << "csr sync request error" << std::endl;
                            break;
                    }
                }    
            }
            ...
            virtual void print(std::string indent){
                ...
                for(auto iter = csr_map_table.begin();iter != csr_map_table.end();iter++){
                    ...
                    stream << indent << std::setw(15) << iter->second.csr->get_name() << "\t[0x" << fillzero(3) << outhex(iter->first) ;
                    stream << ", " << (iter->second.readonly ? "RO" : "RW") << "] = 0x" << fillzero(8) << outhex(iter->second.csr->read());
                    stream << std::endl;
                }
